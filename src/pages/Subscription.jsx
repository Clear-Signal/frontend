...
export default function SubscriptionPage() {
  ...
  // update handleSubscribe to accept a plan argument
  async function handleSubscribe(plan) {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_APP_URL}/api/user/subscribe-now/checkout`,
        {
          planId: plan._id,
          promoCode: appliedPromos[plan._id]?.code || null,
        },
              ...
              <div className="flex flex-col h-full justify-between">
                  ...
                  <ul className="mt-6 space-y-3 flex-1">
                    {plan.features.map((feature, i) => (
                      <li key={i} className="flex items-start gap-3 text-sm">
                        ...
                        <span className="dark:text-[var(--color-fg)]/90 text-black/90">
                          {feature}
                        </span>
                      </li>
                    ))}
                ...
                <div>
                  <div className="mt-6">
                    ...
                    {promoForThisPlan && (
                      <div className="mt-2 text-sm text-[var(--color-warning)] flex items-center justify-between">
                        <div>
                          Applied <strong>{promoForThisPlan.code}</strong> —{" "}
                          {promoForThisPlan.desc}
                        </div>
                        <button
                          onClick={() => clearPromo(plan._id)}
                  ...
                  <div className="mt-6">
                    ...
                    >
                      {isRedirecting && selected === plan._id
                        ? "Redirecting..."
                        : "Subscribe Now"}
                    </button>
                  </div>
