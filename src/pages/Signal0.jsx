...
export default function Signal0() {
  ...
  const combinedItems = useMemo(() => {
    ...
    // Transform problems to a common format
    const formattedProblems = problems.map(p => ({
      id: p._id,
      title: p.title,
      tags: [p.difficulty, p.category].filter(Boolean), // Creates tags like ["Easy", "Linear Algebra"]
      displayId: p.problemId,
      type: 'problem',
    }));

    // Transform collections to a common format
    const formattedCollections = collections.map(c => ({
      id: c._id,
      title: c.name,
      tags: ['Collection'],
      displayId: c.section, // We'll display the section number
      type: 'collection',
    }));
              ...
              >
                <div className="flex items-start gap-3">
                        ...
                        <span
                          ...
                          className={`text-[11px] font-semibold uppercase tracking-wider px-2 py-1 rounded ${
                            tag === "Easy"
                              ...
                              : tag === "Medium"
                              ? "bg-[#ffb36b]/10 text-[#ffb36b] border border-[#6b3e06]"
                              : tag === "Hard"
                  ...
                  <div className="text-gray-500 text-xs flex flex-col items-end">
                    {/* 6. Display Problem ID or Collection Section */}
